...
async function fetchTeamEnergyData() {
  try {
    ...
    const authResponse = await axios.post(TEAM_ENERGY_AUTH_URL, {
      guestMode: false,
      password: "P0ker123$5!",
    ...
    const chargersResponse = await axios.post(TEAM_ENERGY_CHARGERS_URL, {
      noLatest: 1
    }, {
      headers: {
...
async function fetchEvanChargeData() {
  try {
    ...
    const authResponse = await axios.post(EVAN_CHARGE_AUTH_URL, {
      phone: "+37493709944",
      password: "P0ker123$5!"
    }, {
      headers: {
    ...
    if (chargersData && chargersData.length > 0) {
      console.log("Sample Evan Charge charger:", JSON.stringify(chargersData[0], null, 2));
    }
